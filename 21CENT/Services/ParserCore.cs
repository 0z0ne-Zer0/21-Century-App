    ...
    internal class ParserCore
    {
        public static List<SubCat> CategoryGet(MainCat parrent) //Gets categories from section index
        {
            ...
            Parallel.ForEach(list, I =>
            {
                ...
                result.Add(new SubCat { Title = title, Link = link, Pmid = parrent.Mid });
            });
        ...
        public static List<CatalogItem> CatalogGet(string url) //Catalog parser
        {
            ...
            Parallel.ForEach(list, item =>
            {
                ...
                result.Add(new CatalogItem { Name = name, Link = link, Isinstock = stock, Isdiscount = discount, Oldprice = (float?)old, Price = (float?)cur }); //Adding item
            });
