...
namespace Code
{
    internal class ParseLibs
    {
        ...
        private PGRSQLWorker worker = new(IP: "192.168.0.110");

        public ParseLibs()
        {
            baseCat = worker.Read<string>("SELECT URL FROM maincat");
        ...
        public void CategoryParser(List<List<Tuple<string, string>>> subCat) //Category parse
        {
            ...
            for (int i = 0; i < baseCat.Count; i++)
                worker.Insert<string, string>(subCat[i], "Name", "URL", $"INSERT OR IGNORE INTO subcat (Name, URL, MCID) VALUES(@Name, @URL, {i + 1})");
        }
        ...
        public void PageParser(List<Tuple<string>> subCat, int ID)
        {
            ...
            try
            {
                ...
                worker.Insert<int, string>(res, "Pages", "URL", $"UPDATE subcat SET Pages=@Pages WHERE URL=@URL");
        ...
        public void CatalogParser(List<Tuple<string, long, long>> subCat) //Catalog subset parse
        {
            ...
            worker.Insert<string, string, long>(goods, "Name", "URL", "SCID", $"INSERT OR IGNORE INTO goods (Name, URL, SCID) VALUES(@NAME, @URL, @SCID)");
        }
    }
}